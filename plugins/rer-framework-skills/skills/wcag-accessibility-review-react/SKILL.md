...
- The live region must always be rendered in the DOM — never conditionally mount/unmount it in JSX. Use CSS to hide it visually.
...
**React Native:** use a `<FlatList>` or `<ScrollView>` with `accessibilityRole="adjustable"` and provide `accessibilityActions` for increment/decrement; announce the current position via `AccessibilityInfo.announceForAccessibility()`. Respect `AccessibilityInfo.isReduceMotionEnabled()` to skip autoplay.
